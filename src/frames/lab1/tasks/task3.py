...
class Task3Frame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.label = ctk.CTkLabel(
            self, text="Задание 3: Работа со списком", font=style["title_font"]
        )
    ...
    def process_list(self):
        ...
        if not input_text.strip():
            self.output_label.configure(
                text="Ошибка: Пожалуйста, введите список элементов.",
                font=style["main_font"],
            )
            return

        some_list = input_text.split()

        result = ", ".join(some_list[i] for i in range(0, len(some_list), 2))

        self.output_label.configure(
            text=f"Нечётные элементы: {result}", font=style["main_font"]
        )
