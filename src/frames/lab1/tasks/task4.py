...
class Task4Frame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.label = ctk.CTkLabel(
            self, text="Задание 4: Сумма чисел", font=style["title_font"]
        )
    ...
    def process_list(self):
        ...
        result = sum(x for x in some_list if 1 <= x <= 10)

        self.output_label.configure(
            text=f"Сумма чисел (значения которых в диапазоне от 1 до 10): {result}",
            font=style["main_font"],
        )
