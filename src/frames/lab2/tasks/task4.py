...
class Task4Frame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.label = ctk.CTkLabel(
            self, text="Задание 4: Повторение символа R", font=style["title_font"]
        )
        self.label.pack(pady=20, side="top")

        self.input_label = ctk.CTkLabel(
            self, text="Введите количество букв R:", font=style["main_font"]
        )
        self.input_label.pack(pady=5, side="top")

        self.input_entry = ctk.CTkEntry(self, width=200)
        ...
        try:
            ...
            if n < 0:
                self.output_label.configure(
                    text="Ошибка: Введите неотрицательное число."
                )
            else:
                self.output_label.configure(text="R" * n)
        except ValueError:
            self.output_label.configure(text="Ошибка: Введите целое число.")
