...
class Task2Frame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.label = ctk.CTkLabel(
            self,
            text="Задание 2: Фильтрация слов, начинающихся на 'Ли'",
            font=style["title_font"],
        )
        self.label.pack(pady=20, side="top")

        self.input_label = ctk.CTkLabel(
            self, text="Введите строку слов через пробел:", font=style["main_font"]
        )
    ...
    def submit(self):
        ...
        result = [
            word for word in words if len(word) >= 2 and word[0] + word[1] == "Ли"
        ]

        if result:
            self.output_label.configure(
                text="Слова, начинающиеся на 'Ли': " + ", ".join(result)
            )
        else:
            self.output_label.configure(text="Не найдено слов, начинающихся на 'Ли'.")
