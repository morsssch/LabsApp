...
class Task3Frame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.label = ctk.CTkLabel(
            self, text="Задание 3: Слова короче 10 символов", font=style["title_font"]
        )
        self.label.pack(pady=20, side="top")

        self.input_label = ctk.CTkLabel(
            self, text="Введите список слов через пробел:", font=style["main_font"]
        )
    ...
    def submit(self):
        words = self.input_entry.get().split()
        result = [word for word in words if len(word) < 10]

        if result:
            self.output_label.configure(
                text="Слова короче 10 символов:\n" + "\n".join(result)
            )
        else:
            self.output_label.configure(text="Нет слов короче 10 символов.")
