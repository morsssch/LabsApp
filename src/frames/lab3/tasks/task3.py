...
class Task3Frame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.label = ctk.CTkLabel(
            self,
            text="Задание 3: Определение високосного года",
            font=style["title_font"],
        )
        self.label.pack(pady=20, side="top")

        self.input_label = ctk.CTkLabel(
            self, text="Введите год:", font=style["main_font"]
        )
        ...
        self.submit_button = buttonComponent(
            self.button_frame, text="Проверить год", command=self.submit
        )
    ...
    def submit(self):
        try:
            year = int(self.input_entry.get())

            if (year % 4 == 0 and year % 100 != 0) or (year % 400 == 0):
                self.output_label.configure(text="Год високосный")
            else:
                self.output_label.configure(text="Год обычный")
        except ValueError:
            self.output_label.configure(text="Пожалуйста, введите корректный год.")
