...
class Task1Frame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.label = ctk.CTkLabel(
            self,
            text="Задание 1: Нахождение минимального числа",
        ...
        self.input_label_c = ctk.CTkLabel(
            self, text="Введите число c:", font=style["main_font"]
        )
        self.input_label_c.pack(pady=5, side="top")

        self.input_entry_c = ctk.CTkEntry(self, width=400)
        self.input_entry_c.pack(pady=5, side="top")

        self.button_frame = ctk.CTkFrame(self, fg_color="transparent")
        self.button_frame.pack(pady=20, side="top")

        self.submit_button = buttonComponent(
            self.button_frame, text="Найти минимальное", command=self.submit
        )
    ...
    def submit(self):
        try:
            ...
            c = int(self.input_entry_c.get())
            min_value = min(a, b, c)
            self.output_label.configure(text=f"Минимальное число: {min_value}")
        except ValueError:
    ...
    def reset(self):
        ...
        self.input_entry_c.delete(0, ctk.END)
