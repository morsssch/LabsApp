...
class Task5Frame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.label = ctk.CTkLabel(
            self,
            text="Задание 5: Умножение элементов каждой строки",
        ...
        self.multiply_rows_button = buttonComponent(
            self.button_frame, text="Умножить по строкам", command=self.multiply_rows
        )
        self.multiply_rows_button.pack(side="left", padx=5)
    ...
    def multiply_rows(self):
        matrix = self.parse_matrix()
        if matrix is None:
            self.display_error("Ошибка: Введите корректную матрицу")
            return
        result = [self.multiply_row(row) for row in matrix]
        self.update_output(
            f"Результат умножения по строкам:\n{', '.join(map(str, result))}"
        )
