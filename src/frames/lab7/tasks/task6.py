...
class Task6Frame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.label = ctk.CTkLabel(
            self,
            text="Задание 6: Замена чётных элементов матрицы на ноль",
        ...
        self.byposition_button = buttonComponent(
            self.button_frame,
            text="Заменить по позиции",
            command=self.replace_even_elements_by_position,
        )
        self.byposition_button.pack(side="left", padx=5)

        self.byvalue_button = buttonComponent(
            self.button_frame,
            text="Заменить по значению",
            command=self.replace_even_elements_by_value,
        )
        self.byvalue_button.pack(side="left", padx=5)
    ...
    def replace_even_elements_by_position(self):
    ...
    def replace_even_elements_by_value(self):
        matrix = self.parse_matrix()
        if matrix is None:
            self.display_error()
            return
