...
class Task8Frame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.label = ctk.CTkLabel(
            self,
            text="Задание 8: Поменять первую и последнюю строки матрицы",
        ...
        self.swap_button = buttonComponent(
            self.button_frame, text="Поменять строки", command=self.swap_rows
        )
        ...
        self.output = ctk.CTkTextbox(self, width=480, height=150, font=("Consolas", 12))
    ...
    def parse_matrix(self):
        try:
            ...
            if len(set(len(row) for row in matrix)) > 1:
                raise ValueError(
                    "Все строки должны содержать одинаковое количество чисел."
                )
            return matrix
        except Exception:
            return None
